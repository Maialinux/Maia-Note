...
class Editor():
    ...
    def SetEditorTexto(self):
        ...
        self.painelLat.pack(side=LEFT,fill=Y,expand=0)

        self.painel = Frame(self.root,highlightbackground="#000000", highlightcolor="#000000")
        self.painel.pack(side=LEFT,fill=BOTH,expand=1)
        ...
        self.textArea = Text(self.painel,yscrollcommand=self.scrollBar_V.set, 
                             foreground="#E7E7E7", background="#131313", font=("Terminus",13),
        ...
        self.textArea.bind('<Return>', lambda e:self.Incremento(0))
        self.textArea.bind('<BackSpace>',lambda e:self.Decremento(0)) 
       

    def SetNumeroLinha(self):
        global posicao
        cursor = self.textArea.index(INSERT)
        posicao = math.floor(float(cursor))
        #Insere data e hora na posição do cursor
        #self.textArea.insert(posicao,dataEhora)
        self.numeroLinha = Label(self.painelLat,text=str(posicao),font=("Terminus",13))
        self.numeroLinha.grid(row=posicao,column=0)
        
        
    def Incremento(self,e):
        cursor = self.textArea.index(INSERT)
        posicao = math.floor(float(cursor))
        posicao +=1
        print(posicao)
        #self.numeroLinha["text"] = str(posicao)
        self.numeroLinha["text"] += "\n"+str(posicao)
        
        

    def Decremento(self,e):
        cursor = self.textArea.index(INSERT)
        posicao = math.floor(float(cursor))
        ...
        if posicao > 1:
            posicao -=1
            print(posicao)
            self.numeroLinha["text"] = self.numeroLinha["text"][:-1]
            if posicao == 1:
                self.numeroLinha["text"] = "1"
            else:
                cursor = self.textArea.index(INSERT)
                posicao = math.floor(float(cursor))  
                self.numeroLinha["text"] = " "
